...
pub fn extract_cli_json(sh: &Shell) -> Result<CliCommand> {
    let json_output = cmd!(
        sh,
        "cargo run --features=docgen -p bcvk -- internals dump-cli-json"
    )
    .read()
...
fn discover_man_page_mappings(
    ...
    for entry in fs::read_dir(man_dir)? {
        ...
        {
            // Check if there's a section number (e.g., bcvk-install.8.md)
            let cmd_part = if let Some((name, _section)) = cmd_part.rsplit_once('.') {
                name
            } else {
                cmd_part
            };
...
pub fn generate_man_pages(sh: &Shell) -> Result<()> {
    ...
    for entry in fs::read_dir(man_src_dir)? {
        ...
        // Handle both files with and without section numbers
        let (base_name, section) = if let Some((name, section_str)) = filename.rsplit_once('.') {
            if let Ok(section_num) = section_str.parse::<u8>() {
                // Has section number
                (name, section_num)
            } else {
                // No section number, use default section 8 for executables
                (filename, 8)
            }
        } else {
            // No section number, use default section 8 for executables
