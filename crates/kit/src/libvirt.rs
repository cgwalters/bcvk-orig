...
impl LibvirtOpts {
    pub fn run(self) -> Result<()> {
        match self.subcommand {
            LibvirtCommand::SyncCloudImage { os, force } => sync(self.connection, &os, force),
            LibvirtCommand::InstallFromSRB(opts) => opts.run(self.connection),
            LibvirtCommand::List => list_vms(self.connection),
        }
    }
}
...
#[instrument]
fn sync(connection: LibvirtConnection, os: &OperatingSystem, force: bool) -> Result<()> {
