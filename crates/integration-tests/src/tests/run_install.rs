...
/// Test run-install in test mode
pub fn test_run_install_test_mode() {
    let bck = get_bck_command().unwrap();
    ...
    println!("Running test mode installation to: {}", disk_path.display());

    // Run in test mode with shorter timeout
    let output = Command::new("timeout")
        .args([
            "120s", // 2 minute timeout for test mode
            &bck,
            "run-install",
            "--test-mode",
            "quay.io/centos-bootc/centos-bootc:stream10",
        ...
        .output()
        .expect("Failed to run bootc-kit run-install in test mode");
    ...
    if output.status.code() == Some(124) {
        panic!(
            "Test mode timed out after 2 minutes. This may indicate a hanging virtiofsd process:\nstdout: {}\nstderr: {}", 
            stdout, stderr
        );
    }
    ...
    assert!(
        output.status.success(),
        "run-install test mode failed with exit code: {:?}. stdout: {}, stderr: {}",
        output.status.code(),
        stdout,
        stderr
    );

    // In test mode, if we see our success message, the test passes
    assert!(
        stdout.contains("SUCCESS: Test installation completed!")
            || stderr.contains("SUCCESS: Test installation completed!"),
        "Test mode VM did not complete successfully"
    );

    println!("Test mode installation completed successfully");
}

/// Test that virtiofsd startup validation works correctly
pub fn test_virtiofsd_startup_validation() {
    let bck = get_bck_command().unwrap();
    ...
    println!("Testing virtiofsd startup validation with debug mode");

    // Run in debug mode to test virtiofsd startup without full installation
    let output = Command::new("timeout")
        .args([
            "30s", // Short timeout for debug mode
            &bck,
            "run-install",
            "--debug", // This should trigger debug mode and drop into shell after virtiofsd starts
            "--test-mode",
            "quay.io/fedora/fedora-bootc:42",
            disk_path.to_str().unwrap(),
        ])
        .output()
        .expect("Failed to run bootc-kit run-install in debug mode");

    let stdout = String::from_utf8_lossy(&output.stdout);
    let stderr = String::from_utf8_lossy(&output.stderr);

    println!("Virtiofsd validation output:");
    println!("stdout:\n{}", stdout);
    println!("stderr:\n{}", stderr);

    // Check that virtiofsd socket was properly detected/created
    assert!(
        stdout.contains("Virtiofsd socket ready") 
        || stderr.contains("Virtiofsd socket ready") 
        || stdout.contains("Using container storage")  // Alternate success indicator
        || stderr.contains("Using container storage"),
        "No virtiofsd socket ready message found. This indicates the socket waiting fix may not be working. stdout: {}, stderr: {}",
        stdout, stderr
    );

    // Should not contain socket timeout errors
    assert!(
        !stdout.contains("Timeout waiting for virtiofsd socket") 
        && !stderr.contains("Timeout waiting for virtiofsd socket"),
        "Virtiofsd socket timeout detected - socket waiting is not working properly. stdout: {}, stderr: {}",
        stdout, stderr
    );

    println!("Virtiofsd startup validation test passed");
}
